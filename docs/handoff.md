...
- **launchd hands the job a BARE environment** (no `HOME`, `PATH=/usr/bin:/bin:/usr/sbin:/sbin`). `lib.sh`'s `$HOME/...` paths and node (nvm-managed on some Macs, not in `/opt/homebrew/bin`) then fail with `node: command not found` + `board fetch failed` every tick, while a manual `bash tick.sh` works fine because your interactive shell has node on PATH. `install-watcher.sh` bakes `HOME` + node's dir into the plist `EnvironmentVariables`; if you change how node is installed, re-run `install-watcher.sh install`. `orchestration/doctor.sh` catches this and every other bring-up prerequisite; full setup is `docs/bring-up.md`.
