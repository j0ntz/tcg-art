...
case "${1:-}" in
  ...
  status)
    ...
    if [ "$status" = "Blocked" ]; then add_label "$issue" blocked; else remove_label "$issue" blocked; fi
  ...
  *)
    echo "usage: board.sh {list-pending | item-id <issue#> | status <issue#> <Pending|Running|Coded|Testing|Review|Address|Reviewed|Land|Landed|Blocked> | has-label <issue#> <label> | add-label <issue#> <label> | remove-label <issue#> <label>}"; exit 2 ;;
esac
