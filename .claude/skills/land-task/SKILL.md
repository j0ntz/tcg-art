...
<step id="5" name="Done">`bash $ORCH_DIR/board.sh status <n> Done`. Comment on the issue (`<!-- orch -->`): what merged and how you resolved each conflict. Then `bash $ORCH_DIR/release-after-land.sh start <n> <pr>`: it returns at once, is a no-op for a tenant without a release channel, and otherwise builds a new APK in its own tmux session when merged main's native fingerprint differs from the installed APK's, posting the link (or the build failure) on the issue itself. Do not wait for it. Print a one-line summary, then stop.</step>
