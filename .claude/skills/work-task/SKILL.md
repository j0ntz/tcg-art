...
Parse `<n>`. `gh issue view <n> --repo $ORCH_REPO --json title,labels`; `bash $ORCH_DIR/task-spec.sh fetch <n>`; then Read `$ORCH_TASK_DIR/issue.md` and `$ORCH_TASK_DIR/comments.json` (`spec-on-disk`). The FLAVOR is in the labels: `research` / `design` / `instructions` / `chore` (any present = doc/ops kind; none = code). Resolve any PR: `pr=$(gh pr list --repo $ORCH_REPO --head ${ORCH_BRANCH_PREFIX}<n> --state open --json number -q '.[0].number')`.
- If (`pr` exists AND it has UNRESOLVED review threads (Read `$ORCH_TASK_DIR/review-comments.json`)) OR the issue has UNCONSUMED human comments: **ADDRESS** mode.
...
- **code** (no flavor): implement to the issue's definition of done following CLAUDE.md; `git commit`; `$ORCH_BUILD_CMD` (sfw if needed) MUST pass, and on a mobile tenant the local run CLAUDE.md prescribes (`verify-android.sh local`) MUST pass too (its `SCREENSHOT_URL=` lines, when present, are the image URLs to embed in the PR body; raw links into a private repo do not render there); push; open a PR (`gh pr create --repo $ORCH_REPO --base main --head ${ORCH_BRANCH_PREFIX}<n> --title "<concise>" --body-file <file>` whose body starts with `Closes #<n>`).
