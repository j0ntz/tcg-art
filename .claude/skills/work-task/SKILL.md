...
Parse `<n>`. `D=/tmp/orch-$ORCH_SLUG-<n>; mkdir -p $D; gh issue view <n> --repo $ORCH_REPO --json title,labels; gh issue view <n> --repo $ORCH_REPO --json body -q .body > $D/issue.md; gh api repos/$ORCH_REPO/issues/<n>/comments --paginate > $D/comments.json`, then READ `$D/issue.md` and `$D/comments.json` with the Read tool (see `no-truncated-reads`). The FLAVOR is in the labels: `research` / `design` / `instructions` / `chore` (any present = doc/ops kind; none = code). Resolve any PR: `pr=$(gh pr list --repo $ORCH_REPO --head ${ORCH_BRANCH_PREFIX}<n> --state open --json number -q '.[0].number')`.
- If (`pr` exists AND it has UNRESOLVED review threads (`gh api repos/$ORCH_REPO/pulls/<pr>/comments --paginate > $D/review-comments.json`, Read it)) OR the issue has UNCONSUMED human comments: **ADDRESS** mode.
