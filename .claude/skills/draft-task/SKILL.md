---
name: draft-task
description: Front door for the orchestration. turn a task intent (free text or an idea) into a well-structured GitHub issue, classify its flavor, queue it on the tenant's board as Pending, and apply the right labels so the orch picks it up. Invoked as `/draft-task <intent...>`. Repo-local.
---

<goal>Turn a task intent into a queued orch task: a structured GitHub issue (Goal / Deliverables / Constraints / Acceptance), the right flavor and automation labels, added to the tenant's project board and set Pending. Present the draft for confirmation before queueing unless told to queue directly.</goal>

<rules>
<rule id="tenant-env">Tenant values come from the `ORCH_*` env (`$ORCH_REPO`, `$ORCH_OWNER`, `$ORCH_PROJECT_NUMBER`, `$ORCH_DIR`). If `$ORCH_REPO` is empty (interactive use), run `eval "$(bash ~/git/site-orch/env.sh)"` from the repo root first.</rule>
