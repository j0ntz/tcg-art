...
Parse `<n>`. `gh issue view <n> --repo $ORCH_REPO --json title,body,labels`. Resolve any PR: `pr=$(gh pr list --repo $ORCH_REPO --head ${ORCH_BRANCH_PREFIX}<n> --state open --json number -q '.[0].number')`. A PR present means doc/repo-bound; no PR means research-only or board-ops.
</step>

<step id="2" name="Check">
- **Doc task (PR present)**: read `gh pr diff <pr> --repo $ORCH_REPO` and the doc(s). Verify EACH deliverable named in the issue is delivered; spot-check that cited facts and links resolve and the design is sound and self-consistent; if code changed, `$ORCH_BUILD_CMD` must pass. Anything missing, wrong, or unsound is a change request (file:line).
